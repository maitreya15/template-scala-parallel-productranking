...
import import org.apache.predictionio.controller.P2LAlgorithm
import import org.apache.predictionio.controller.Params
import import org.apache.predictionio.data.storage.BiMap
  ...
  def train(sc: SparkContext, data: PreparedData): ALSModel = {
    ...
    val mllibRatings = data.viewEvents
      ...
      .map { r =>
        ...
        // keep events with valid user and item index
        (u != -1) && (i != -1)
      }.reduceByKey(_ + _) // aggregate all play events of same user-item pair
      .map { case ((u, i), v) =>
        // MLlibRating requires integer index for user and item
        MLlibRating(u, i, v)
      }
